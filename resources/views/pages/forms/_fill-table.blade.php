@php
    ...
    $columnWidthClass = fn (string $type) => match ($type) {
        'textarea' => 'w-56',
        'select' => 'w-44',
        default => 'w-36',
    };
            ...
            <thead class="bg-zinc-50 dark:bg-zinc-800/40">
                ...
                <tr>
                    @foreach ($form->columns as $column)
                        <th class="px-4 py-2 text-left font-medium text-zinc-700 dark:text-zinc-300 border-b border-zinc-200 dark:border-zinc-700 {{ $columnBorderClass($column->id) }} {{ $columnWidthClass($column->type) }}">
